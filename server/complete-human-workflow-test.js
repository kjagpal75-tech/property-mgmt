...
/**
 * Complete human-like search workflow with mouse movement + randomized typing
 */
async function testCompleteHumanWorkflow() {
  let browser;
  let page;
  
  try {
    console.log('🚀 COMPLETE HUMAN-LIKE SEARCH WORKFLOW...');
    console.log('🔍 Mouse movement → Click → Randomized typing → Submit');
    
    browser = await puppeteer.launch({
      headless: false,  // Show browser so you can see everything
      args: [
        '--no-sandbox',
    ...
    // === STEP 1: MOUSE MOVEMENT TO SEARCH FIELD ===
    console.log('\n=== STEP 1: MOUSE MOVEMENT TO SEARCH FIELD ===');
    ...
    if (inputBounds) {
      console.log('🖱️ Moving mouse to search field like a human...');
      
      // Move mouse to search input with natural movement
      ...
      // Add slight human-like movement
      await page.mouse.move(
        inputBounds.x + 70,  // Slight movement
        inputBounds.y + inputBounds.height / 2 - 10  // Slight upward movement
      );
      ...
      console.log('✅ Mouse positioned at search field');
    }
    
    // === STEP 2: CLICK ON SEARCH FIELD ===
    console.log('\n=== STEP 2: CLICK ON SEARCH FIELD ===');
    ...
    console.log('✅ Clicked on search field - should be focused now');
    
    // === STEP 3: RANDOMIZED TYPING ===
    console.log('\n=== STEP 3: RANDOMIZED TYPING (500ms-2s variance) ===');
    ...
    console.log('🔤 Typing address with 500ms-2s random variance...');
    
    for (let i = 0; i < address.length; i++) {
      const char = address[i];
      
      // Randomized delay: 500ms to 2000ms (500ms + Math.random() * 1500ms)
    ...
    // === STEP 4: WAIT AND SUBMIT ===
    console.log('\n=== STEP 4: WAIT AND SUBMIT ===');
    
    // Human-like waiting before submitting
    const submitWait = 2000 + Math.random() * 2000; // 2-4 seconds
    ...
    // Take screenshot
    await page.screenshot({ path: 'complete-human-workflow.png' });
    console.log('✅ Screenshot saved: complete-human-workflow.png');
    
    if (searchResult.hasPropertyData && searchResult.hasAddress && !searchResult.hasError && !searchResult.isHomepage) {
      console.log('🎉 SUCCESS! Complete human-like workflow worked!');
      ...
      return { 
        success: true, 
        method: 'Complete Human-Like Workflow',
        propertyData,
      ...
      console.log('❌ Complete human-like workflow failed');
    ...
    console.error('❌ Complete human-like workflow error:', error.message);
...
/**
 * Run complete human-like workflow test
 */
async function runCompleteHumanWorkflowTest() {
  console.log('=== COMPLETE HUMAN-LIKE WORKFLOW TEST ===');
  console.log('🖱️ Mouse movement → Click → Randomized typing → Submit');
  console.log('🔍 This will show you the complete human-like search workflow!');
  
  const result = await testCompleteHumanWorkflow();
  
  if (result.success) {
    console.log('🎉 Complete Human-Like Workflow Test SUCCESS!');
    console.log('✅ Mouse movement worked!');
    console.log('✅ Randomized typing worked!');
    console.log('✅ Property data extracted successfully!');
    console.log(`🏠 Market Value: $${result.propertyData.price.toLocaleString()}`);
  } else {
    console.log('❌ Complete Human-Like Workflow Test FAILED!');
    console.log(`🔧 Reason: ${result.reason}`);
  }
  
  console.log('\n=== Complete Human-Like Workflow Test Complete ===');
}

// Run the test if this file is executed directly
if (require.main === module) {
  runCompleteHumanWorkflowTest().catch(console.error);
}

module.exports = {
  testCompleteHumanWorkflow,
  runCompleteHumanWorkflowTest
};
